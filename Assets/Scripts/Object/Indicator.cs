...
public class Indicator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("InteractionObject component is missing!");
            enabled = false;
            return;
        }
        SetVisibility(false);
    ...
    {
        Vector2 objectSize = interactionObject.GetSize();
        Vector3 objectPosition = transform.position;

        Vector3 viewportMin = mainCamera.WorldToViewportPoint(objectPosition - (Vector3)objectSize / 2);
        Vector3 viewportMax = mainCamera.WorldToViewportPoint(objectPosition + (Vector3)objectSize / 2);

        bool newIsVisible = !(viewportMax.x < 0 || viewportMin.x > 1 ||
                                      viewportMax.y < 0 || viewportMin.y > 1 ||
                                      viewportMin.z < 0);
